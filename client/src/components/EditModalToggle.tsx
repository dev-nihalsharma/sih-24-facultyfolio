...
export default function EditTimetableModal({ show, onClose, onSuccess }: EditTimetableModalProps) {
  ...
  const generateTimeSlots = (startTime: string, endTime: string) => {
    ...
    for (let current = start; current < end; current++) {
      const formattedStart = String(Math.floor(current)).padStart(2, '0') + ':00';
      const formattedEnd = String(Math.floor(current + 1)).padStart(2, '0') + ':00';
