...
export function ScheduleListComponent() {
  ...
  if (loading) {
    return (
      <div className='p-4 sm:p-6 md:p-8 border rounded-lg dark:bg-gray-900'>
        <h2 className='text-2xl font-semibold text-gray-900 dark:text-white mb-4'>Today&apos;s Schedule</h2>
  ...
  const timetables = schedules.filter((schedule) => schedule.timeTable !== null);

  return (
    <div className='p-4 sm:p-6 md:p-8   dark:bg-gray-900 border rounded-lg'>
      <h2 className='text-2xl font-semibold text-gray-900 dark:text-white mb-4 '>Today&apos;s Schedule</h2>
      <div className='flex flex-row w-full justify-between gap-4'>
              ...
              {events.map((schedule) => (
                ...
                >
                  ...
                  <Table.Cell className='px-4 py-3'>
                    <div>
                      <strong>Description:</strong> {schedule.event?.description || 'No description'}
                      <br />
                      <strong>Date:</strong>{' '}
                      {schedule.event?.date ? formatDate(new Date(schedule.event.date).getTime()) : 'No date'}
                      <br />
                      <strong>Venue:</strong> {schedule.event?.venue || 'No venue'}
                    </div>
                  </Table.Cell>
                  <Table.Cell className='px-4 py-3'>
                    <button
        ...
        {/* Timetables Table */}
        <div className='relative w-full overflow-auto border rounded-lg mb-6'>
          <Table hoverable className='min-w-full dark:bg-gray-800'>
            <Table.Head className=' dark:bg-gray-700'>
            ...
            <Table.Body className='divide-y divide-gray-200 dark:divide-gray-700'>
              {timetables.map((schedule) => (
                ...
                >
                  ...
                  <Table.Cell className='px-4 py-3'>
                    <div>
                      <strong>Room:</strong> {schedule.timeTable?.room || 'No room available'}
                      <br />
                      <strong>Time:</strong> {schedule.timeTable?.startTime || 'No start time'} -{' '}
